...
class CreateDownloadCategoriesTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('download_categories', function (Blueprint $table) {
            ...
            $table->foreign('existing_record_id')->references('id')->on('download_categories')->onUpdate('cascade')->onDelete('cascade');
            $table->foreign('parent_id')->references('id')->on('download_categories')->onUpdate('cascade')->onDelete('cascade');
