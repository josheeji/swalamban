...
@section('title', 'Stock Watch - manage')
@section('styles')
        ...
        $(".defaultTable").on("click", ".change-status", function() {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        type: "POST",
                        url: "{{ route('admin.stock-watch.change-status') }}",
                        data: {
                            'id': id,
                        },
                        dataType: 'json',
        ...
        $(".defaultTable").on("click", ".delete", function() {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        type: "POST",
                        url: baseUrl + "/admin/stock-watch" + "/" + id,
                        data: {
                            id: id,
                            _method: 'DELETE'
                        },
...
<div class="subheader py-2 py-lg-4  subheader-solid " id="kt_subheader">
    <div class=" container-fluid  d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
        ...
        <div class="d-flex align-items-center">
            @can('master-policy.perform', ['stock-info', 'add'])
            <a title="Create Content" href="{{ route('admin.stock-watch.create') }}" class="btn btn-default btn-outline-success font-weight-bolder">Create New</a>
            @endif
        </div>
        <!--end::Toolbar-->
    </div>
            ...
            <div class="card-body">
                <div class="datatable datatable-bordered datatable-head-custom datatable-default datatable-primary datatable-loaded" id="kt_datatable">
                    <table class="table datatable-column-search-inputs defaultTable">
                        ...
                        <tbody id="sortable">
                            @can('master-policy.perform', ['stock-info', 'edit'])
                            @php $edit_access = 1; @endphp
                            @endcan
                            @can('master-policy.perform', ['stock-info', 'delete'])
                            @php $delete_access = 1; @endphp
                            @endcan
                            @can('master-policy.perform', ['stock-info', 'changeStatus'])
                            @php $status_access = 1; @endphp
                            @endcan
                            @foreach($stockInfos as $key => $data)
                            <tr id="item-{{ $data->id }}">
                                <td>{{ $key+ 1 }}</td>
                                <td>NPR: {{ $data->paidup_value }}/-</td>
                                <td>NPR: {{ $data->maximum }}/-</td>
                                <td>NPR: {{ $data->minimum }}/-</td>
                                <td>NPR: {{ $data->closing }}/-</td>
                                <td>{{ $data->traded_share }}/-</td>
                                <td>{{ $data->published_at }}</td>
                                <td>
                                    @if(isset($status_access) && $status_access == true)
                                    <a title="Change Status" href="javascript:void(0)" class="btn btn-primary btn-icon change-status btn-circle" id="{{ $data->id }}">
                                        @if($data->is_active == 1)
                                ...
                                <td class="text-center">
                                    ...
                                    <a title="Edit" href="{{ route('admin.stock-watch.edit', $data->id) }}" class="btn btn-icon btn-light btn-circle"><i class="la la-pencil"></i></a>
                                    @endif
                                    @if(isset($delete_access) && $delete_access == true)
                                    <a title="Delete" href="javascript:void(0)" id="{{ $data->id }}" class="btn btn-icon btn-danger delete btn-circle"><i class="la la-trash"></i></a>
                                    @endif
                                </td>
                            </tr>
                            @endforeach
