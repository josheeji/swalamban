        ...
        $(".defaultTable").on("click", ".change-status", function() {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        type: "POST",
                        url: "{{ url('admin/popup/change-status') }}",
                        data: {
                            'id': id,
                        },
                        dataType: 'json',
...
<div class="subheader py-2 py-lg-4  subheader-solid " id="kt_subheader">
    <div class=" container-fluid  d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
        ...
        <div class="d-flex align-items-center">
            @can('master-policy.perform', ['popup', 'add'])
            <a title="Create Content" href="{{ route('admin.popup.create') }}" class="btn btn-default btn-outline-success font-weight-bolder">Create New</a>
            @endif
        </div>
        <!--end::Toolbar-->
    </div>
            ...
            <div class="card-body">
                <div class="datatable datatable-bordered datatable-head-custom datatable-default datatable-primary datatable-loaded" id="kt_datatable">
                    <table class="table datatable-column-search-inputs defaultTable">
                        ...
                        <tbody id="sortable">
                            @can('master-policy.perform', ['popup', 'edit'])
                            @php $edit_access = 1; @endphp
                            @endcan
                            @can('master-policy.perform', ['popup', 'delete'])
                            @php $delete_access = 1; @endphp
                            @endcan
                            @can('master-policy.perform', ['popup', 'changeStatus'])
                            ...
                            <tr id="item-{{ $popup->id }}">
                                ...
                                <td>{{ $popup->title }}</td>
                                <td>
                                    @if(file_exists('storage/'.$popup->image) && !empty($popup->image))
                                        <a href="{!! asset('storage/'.$popup->image) !!}" target="_blank"><i class="la la-download"></i></a>
                                    @elseif($popup->type == 'Link')
                                        <a href="{!! $popup->image !!}"><i class="la la-link"></i></a>
                                    @endif
                                </td>
                                <td>
                                    @if(isset($status_access) && $status_access == true)
                                    <a title="Change Status" href="javascript:void(0)" class="btn btn-primary btn-icon change-status btn-circle" id="{{ $popup->id }}">
                                ...
                                <td>
                                    ...
                                    <a href="{{ route('admin.popup.edit', $popup->id) }}" class="btn btn-icon btn-light btn-circle"><i class="la la-pencil"></i></a>
                                    @endif
                                    @if(isset($delete_access) && $delete_access == true)
                                    <a href="javascript:void(0)" id="{{  $popup->id  }}" class="btn btn-icon btn-danger delete btn-circle"><i class="la la-trash"></i></a>
                                    @endif
                                </td>
                            </tr>
                            @endforeach
