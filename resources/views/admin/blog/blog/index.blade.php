        ...
        $(".defaultTable").on("click", ".change-status", function() {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        type: "POST",
                        url: "{{ route('admin.stories.change-status') }}",
                        data: {
                            'id': id,
                        },
                        dataType: 'json',
...
<div class="subheader py-2 py-lg-4  subheader-solid " id="kt_subheader">
    <div class=" container-fluid  d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
        ...
        <div class="d-flex align-items-center">
            ...
            <a title="Create Content" href="{{ route('admin.stories.create') }}" class="btn btn-default btn-outline-success font-weight-bolder">Create New</a>
            @endif
        </div>
        <!--end::Toolbar-->
    </div>
...
{{--                                <th>Category</th>--}}
                        ...
                        <tbody id="sortable">
                            ...
                            <tr id="item-{{ $blog->id }}">
                                <td>{{ $key+ 1 }}</td>
                                <td>{{ $blog->title }}</td>
{{--                                <td>{{ isset($blog->category_id) && isset($blog->category) ? $blog->category->title : 'N.A.' }}</td>--}}
                                <td>{{ $blog->published_date }}</td>
                                <td>
                                    @if(isset($status_access) && $status_access == true)
                                    <a title="Change Status" href="javascript:void(0)" class="btn btn-primary btn-icon change-status btn-circle" id="{{ $blog->id }}">
                                        @if($blog->is_active == 1)
                                ...
                                <td class="text-center">
                                    ...
                                    <a title="Edit" href="{{ route('admin.stories.edit',$blog->id) }}" class="btn btn-icon btn-light btn-circle"><i class="la la-pencil"></i></a>
                                    @endif
                                    @if(isset($delete_access) && $delete_access == true)
                                    <a title="Delete" href="javascript:void(0)" id="{{ $blog->id  }}" class="btn btn-icon btn-danger delete btn-circle"><i class="la la-trash"></i></a>
                                    @endif
                                </td>
                            </tr>
                            @endforeach
