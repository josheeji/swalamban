...
@section('title', 'Product Category- manage')
@section('styles')
        ...
        $(".defaultTable").on("click", ".change-status", function() {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        type: "POST",
                        url: "{{ route('admin.account-type-category.change-status') }}",
                        data: {
                            'id': id,
                        },
                        dataType: 'json',
...
<div class="subheader py-2 py-lg-4  subheader-solid " id="kt_subheader">
    <div class=" container-fluid  d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
        ...
        <div class="d-flex align-items-center">
            {{-- <a title="Create Content" href="{{ route('admin.account-type.export') }}"
                class="btn btn-outline-warning font-weight-bolder mr-2">Export</a>--}}
            @can('master-policy.perform', ['account-type-category', 'add'])
            <a title="Create Content" href="{{ route('admin.account-type-category.create') }}"
                        ...
                        <tbody id="sortable">
                            ...
                            <tr id="item-{{ $item->id }}">
                                ...
                                <td>
                                    ...
                                    <img src="{{ asset('storage/thumbs/'.$item->image) }}" class="displayimage" alt=""
                                        width="120">
                                    @endif
                                </td>
                                @if(SettingHelper::setting('visible_in') == 1)
                                ...
                                <td class="text-center">
                                    ...
                                    <a title="Edit" href="{{ route('admin.account-type-category.edit',$item->id) }}"
                                        class="bg-slate-700 btn btn-success btn-icon btn-circle legitRipple">
                                        <i class=" la la-pencil"></i>
                                    </a>
                                    @endif
                                    {{-- <a href="{{ route('admin.account-type.enquiry', $item->id) }}"
                                        class="btn btn-info btn-icon btn-circle" title="Enquiries"><i
