...
@section('title', 'Financial Report - manage')
@section('styles')
    ...
    $(function() {
        ...
        $('#sortable').sortable({
            ...
            update: function(event, ui) {
                ...
                var url = "{{ url('admin/financial-report/sort') }}";
                $.ajax({
                    type: "POST",
                    url: url,
                    datatype: "json",
        ...
        $(".defaultTable").on("click", ".change-status", function() {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        type: "POST",
                        url: "{{ route('admin.financial-report.change-status') }}",
                        data: {
                            'id': id,
                        },
                        dataType: 'json',
        ...
        $(".defaultTable").on("click", ".delete", function() {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        type: "POST",
                        url: baseUrl + "/admin/financial-report" + "/" + id,
                        data: {
                            id: id,
                            _method: 'DELETE'
                        },
...
<div class="subheader py-2 py-lg-4  subheader-solid " id="kt_subheader">
    <div class=" container-fluid  d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
        ...
        <div class="d-flex align-items-center flex-wrap mr-2">
            <!--begin::Page Title-->
            <h5 class="text-dark font-weight-bold mt-2 mb-2 mr-5">Financial Reports</h5>
            <!--end::Page Title-->
            <!--begin::Actions-->
        ...
        <div class="d-flex align-items-center">
            @can('master-policy.perform', ['financial-report', 'add'])
            <a title="Create Content" href="{{ route('admin.financial-report.create') }}"
            ...
            <div class="card-body">
                <div class="datatable datatable-bordered datatable-head-custom datatable-default datatable-primary datatable-loaded"
                    ...
                    <table class="table datatable-column-search-inputs defaultTable">
                        ...
                        <tbody id="sortable">
                            @can('master-policy.perform', ['financial-report', 'edit'])
                            @php $edit_access = 1; @endphp
                            @endcan
                            @can('master-policy.perform', ['financial-report', 'delete'])
                            @php $delete_access = 1; @endphp
                            @endcan
                            @can('master-policy.perform', ['financial-report', 'changeStatus'])
                            ...
                            <tr id="item-{{ $download->id }}">
                                ...
                                <td>
                                    @if ($download->company_id == 1)
                                    <span class="badge badge-warning">Surya Jyoti Life</span>
                                    @elseif ($download->company_id == 2)
                                    <span class="badge badge-secondary">Surya Life</span>
                                    @elseif ($download->company_id == 3)
                                    <span class="badge badge-primary">Jyoti Life</span>
                                    @else
                                    <span class="badge badge-danger">N/A</span>
                                    @endif
                                </td>
                                <td>{{ $download->published_date }}</td>
                                <td>
                                    @if(!empty($download->file))
                                    <a href="{!! asset('storage/'.$download->file) !!}" target="_blank"><i
                                            class="la la-download"></i></a>
                                    @elseif($download->type == 'Link')
                                    <a href="{!! $download->file !!}" target="_blank"><i class="icon-link"></i></a>
                                    @endif
                                </td>
                                <td>
                                ...
                                <td class="text-center">
                                    ...
                                    <a title="Edit" href="{{ route('admin.financial-report.edit',$download->id) }}"
                                        class="bg-slate-700 btn btn-success btn-icon btn-circle legitRipple">
                                        <i class=" la la-pencil"></i>
                                    </a>
                                    @endif
                                    @if(isset($delete_access) && $delete_access == true)
                                    <a title="Delete" href="javascript:void(0)" id="{{ $download->id  }}"
