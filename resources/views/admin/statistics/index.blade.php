...
@section('title', 'Statistics - manage')
@section('styles')
    ...
    $(function() {
        ...
        $('#sortable').sortable({
            ...
            update: function(event, ui) {
                ...
                var url = "{{ url('admin/statistics/sort') }}";
                $.ajax({
                    type: "POST",
                    url: url,
                    datatype: "json",
        ...
        $(".defaultTable").on("click", ".change-status", function() {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        type: "POST",
                        url: "{{ route('admin.statistics.change-status') }}",
                        data: {
                            'id': id,
                        },
                        dataType: 'json',
        ...
        $(".defaultTable").on("click", ".delete", function() {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        type: "POST",
                        url: baseUrl + "/admin/statistics" + "/" + id,
                        data: {
                            id: id,
                            _method: 'DELETE'
                        },
...
<div class="subheader py-2 py-lg-4  subheader-solid " id="kt_subheader">
    <div class=" container-fluid  d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
        ...
        <div class="d-flex align-items-center">
            @can('master-policy.perform', ['statistics', 'add'])
            <a title="Create Content" href="{{ route('admin.statistics.create') }}" class="btn btn-default btn-outline-success font-weight-bolder">Create New</a>
            @endif
        </div>
        <!--end::Toolbar-->
    </div>
            ...
            <div class="card-body">
                <div class="datatable datatable-bordered datatable-head-custom datatable-default datatable-primary datatable-loaded" id="kt_datatable">
                    <table class="table datatable-column-search-inputs defaultTable">
                        ...
                        <tbody id="sortable">
                            @can('master-policy.perform', ['statistics', 'edit'])
                            @php $edit_access = 1; @endphp
                            @endcan
                            @can('master-policy.perform', ['statistics', 'delete'])
                            @php $delete_access = 1; @endphp
                            @endcan
                            @can('master-policy.perform', ['statistics', 'changeStatus'])
                            ...
                            <tr id="item-{{ $item->id }}">
                                ...
                                <td class="text-center">
                                    ...
                                    <a title="Edit" href="{{ route('admin.statistics.edit',$item->id) }}" class="btn btn-icon btn-light btn-circle"><i class="la la-pencil"></i></a>
                                    @endif
