            ...
            update: function(event, ui) {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        Swal.fire({
                            title: "Success!",
                            text: "Categories has been sorted.",
                            icon: 'success',
                            timer: 2000,
    ...
    $(document).ready(function() {
        $(".defaultTable").on("click", ".change-status", function() {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        type: "POST",
                        url: "{{ route('admin.navs.change-status') }}",
                        data: {
                            'id': id,
                        },
                        dataType: 'json',
        ...
        $('.btn-flush').on('click', function(e) {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    window.location = "{{ route('admin.navs.flush') }}"
                } else {
                    return;
                }
...
<div class="subheader py-2 py-lg-4  subheader-solid " id="kt_subheader">
    <div class=" container-fluid  d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
        ...
        <div class="d-flex align-items-center">
            @can('master-policy.perform', ['nav', 'add'])
            <a title="Create Content" href="{{ route('admin.navs.create') }}" class="btn btn-default btn-outline-success font-weight-bolder mr-2">Create New</a>
            @endif
            @can('master-policy.perform', ['nav', 'add'])
            <a title="Create Branch" href="{{ route('admin.navs.import') }}" class="btn btn-outline-success font-weight-bolder mr-2"><i class="icon-file-plus position-left"></i>Import</a>
            @endif
            @can('master-policy.perform', ['nav', 'delete'])
            <a title="Flush Data" href="javascript:void(0);" class="btn btn-outline-danger btn-flush font-weight-bolder"><i class="la la-trash position-left"></i>Trash</a>
            @endif
        </div>
        <!--end::Toolbar-->
    </div>
            ...
            <div class="card-body">
                <div class="datatable datatable-bordered datatable-head-custom datatable-default datatable-primary datatable-loaded" id="kt_datatable">
                    <table class="table datatable-column-search-inputs defaultTable">
                        ...
                        <tbody id="sortable">
                            @can('master-policy.perform', ['nav', 'edit'])
                            @php $edit_access = 1; @endphp
                            @endcan
                            @can('master-policy.perform', ['nav', 'delete'])
                            @php $delete_access = 1; @endphp
                            @endcan
                            @can('master-policy.perform', ['nav', 'changeStatus'])
                            ...
                            <tr id="item-{{ $item->id }}">
                                ...
                                <td class="text-center">
                                    ...
                                    <a title="Edit" href="{{ route('admin.navs.edit',$item->id) }}" class="btn btn-icon btn-light btn-circle"><i class="la la-pencil"></i></a>
                                    @endif
                                    @if(isset($delete_access) && $delete_access == true)
                                    <a title="Delete" href="javascript:void(0)" id="{{ $item->id  }}" class="btn btn-icon btn-danger delete btn-circle"><i class="la la-trash"></i></a>
                                    @endif
                                </td>
                            </tr>
                            @endforeach
