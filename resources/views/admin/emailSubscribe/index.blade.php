...
<div class="subheader py-2 py-lg-4  subheader-solid " id="kt_subheader">
    <div class=" container-fluid  d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
        ...
        <div class="d-flex align-items-center flex-wrap mr-2">
            <!--begin::Page Title-->
            <h5 class="text-dark font-weight-bold mt-2 mb-2 mr-5">Email Subscription</h5>
            <!--end::Page Title-->
            <!--begin::Actions-->
            ...
            <div class="card-body">
                <div class="datatable datatable-bordered datatable-head-custom datatable-default datatable-primary datatable-loaded"
                    ...
                    <table class="table datatable-column-search-inputs defaultTable">
                        ...
                        <tbody id="sortable">
                            @can('master-policy.perform', ['email-subscribe', 'edit'])
                            @php $edit_access = 1; @endphp
                            @endcan
                            @can('master-policy.perform', ['email-subscribe', 'delete'])
                            @php $delete_access = 0; @endphp
                            @endcan
                            @can('master-policy.perform', ['email-subscribe', 'changeStatus'])
                            @php $status_access = 1; @endphp
                            @endcan
                            @foreach($emailsubscribes as $index=>$item)
                            <tr id="item-{{ $item->id }}">
                                <td>{{ $index+1 }}</td>
                                <td>{{ $item->full_name }}</td>
                                <td>{{ $item->email }}</td>
                                <td class="text-center">
                                    {{-- <a title="View" href="{{ route('admin.email-subscribe.show',$item->id) }}"
                                        class="btn btn-success btn-icon btn-circle legitRipple">
                                        <i class="la la-eye"></i>
                                    </a> --}}
                                    @if(isset($delete_access) && $delete_access == true)
                                    <a title="Delete Content" href="javascript:void(0)" id="{{ $item->id  }}"
