...
@section('title', 'Internal Web - ' . $download->title)
@section('scripts')
<script>
...
<div class="subheader py-2 py-lg-4  subheader-solid " id="kt_subheader">
    <div class=" container-fluid  d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
        ...
        <div class="d-flex align-items-center">
            <a href="{{ route('admin.internal-web.index') }}" class="btn btn-default btn-outline-success font-weight-bolder">Back</span></a>
        </div>
        <!--end::Toolbar-->
    </div>
</div>
...
<div class="d-flex flex-column-fluid">
    ...
    <div class="container-fluid">
        {!! Form::open(array('route' => ['admin.internal-web.update', $download->id],'class'=>'form-horizontal','id'=>'validator', 'files' => 'true','novalidate')) !!}
        <div class="row">
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group">
                            ...
                            <select name="category_id" class="form-control">
                                ...
                                @include('admin.internalWeb.recursive_options', ['parents' => $categories, 'selected_id' => $download->category_id])
                            </select>
                        </div>
                        <div class="form-group">
                            ...
                            <select name="year" class="form-control">
                                ...
                                @foreach (PageHelper::year() as $value)
                                    <option value="{{ $value }}" {{$value == $download->year ? 'selected' : ''}}>
                                        {{ $value }}</option>
                                @endforeach
                            </select>
                        </div>
                        {{-- <div class="form-group">
                            ...
                            <select name="month" class="form-control">
                                ...
                                @foreach (PageHelper::month() as $key => $value)
                                    <option value="{{ $key }}" {{$key == $download->month ? 'selected' : ''}}>
                                        {{ $value }}</option>
                                @endforeach
                            </select>
                        </div> --}}
