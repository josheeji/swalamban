...
@section('title', 'Internal Web - manage')
@section('styles')
    ...
    $(function() {
        ...
        $('#sortable').sortable({
            ...
            update: function(event, ui) {
                ...
                var url = "{{ url('admin/internal-web/sort') }}";
                $.ajax({
                    type: "POST",
                    url: url,
                    datatype: "json",
        ...
        $(".defaultTable").on("click", ".change-status", function() {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        type: "POST",
                        url: "{{ route('admin.internal-web.change-status') }}",
                        data: {
                            'id': id,
                        },
                        dataType: 'json',
        ...
        $(".defaultTable").on("click", ".delete", function() {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        type: "POST",
                        url: baseUrl + "/admin/internal-web/" + id,
                        data: {
                            id: id,
                            _method: 'DELETE'
                        },
...
<div class="subheader py-2 py-lg-4  subheader-solid " id="kt_subheader">
    <div class=" container-fluid  d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
        ...
        <div class="d-flex align-items-center">
            ...
            <a title="Create Content" href="{{ route('admin.internal-web.create') }}" class="btn btn-default btn-outline-success font-weight-bolder">Create New</a>
            @endif
        </div>
        <!--end::Toolbar-->
    </div>
...
{{--                                <th>Published Date</th>--}}
                        ...
                        <tbody id="sortable">
                            @can('master-policy.performArray', [['download', 'download-specific-category'], 'edit'])
                            @php $edit_access = 1; @endphp
                            @endcan
                            @can('master-policy.performArray', [['download', 'download-specific-category'], 'delete'])
                            @php $delete_access = 1; @endphp
                            @endcan
                            @can('master-policy.performArray', [['download', 'download-specific-category'], 'changeStatus'])
...
{{--                                <td>{{ $download->published_date }}</td>--}}
                                ...
                                <td>
                                    ...
                                    <a title="Change Status" href="javascript:void(0)" class="btn btn-primary btn-icon change-status btn-circle" id="{{ $download->id }}">
                                ...
                                <td class="text-center">
                                    ...
                                    <a title="Edit" href="{{ route('admin.internal-web.edit',$download->id) }}" class="btn btn-icon btn-light btn-circle"><i class="la la-pencil"></i></a>
                                    @endif
                                    @if(isset($delete_access) && $delete_access == true)
                                    <a title="Delete" href="javascript:void(0)" id="{{ $download->id  }}" class="btn btn-icon btn-danger delete btn-circle"><i class="la la-trash"></i></a>
                                    @endif
                                </td>
                            </tr>
                            @endforeach
