...
@section('title', 'Team Categories - manage')
@section('styles')
    ...
    $(function() {
        ...
        $('#sortable').sortable({
            ...
            update: function(event, ui) {
                ...
                var url = "{{ url('admin/team-category/sort') }}";
                console.log(data);
                $.ajax({
                    type: "POST",
                    url: url,
        ...
        $(".defaultTable").on("click", ".change-status", function() {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        type: "POST",
                        url: "{{ route('admin.team-category.change-status') }}",
                        data: {
                            'id': id,
                        },
                        dataType: 'json',
        ...
        $(".defaultTable").on("click", ".delete", function() {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        type: "POST",
                        url: baseUrl + "/admin/team-category" + "/" + id,
                        data: {
                            id: id,
                            _method: 'DELETE'
                        },
...
<div class="subheader py-2 py-lg-4  subheader-solid " id="kt_subheader">
    <div class=" container-fluid  d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
        ...
        <div class="d-flex align-items-center flex-wrap mr-2">
            <!--begin::Page Title-->
            <h5 class="text-dark font-weight-bold mt-2 mb-2 mr-5">Team Categories</h5>
            <!--end::Page Title-->
            <!--begin::Actions-->
        ...
        <div class="d-flex align-items-center">
            @can('master-policy.perform', ['team-category', 'add'])
            <a title="Create Content" href="{{ route('admin.team-category.create') }}" class="btn btn-default btn-outline-success font-weight-bolder">Create New</a>
            @endif
        </div>
        <!--end::Toolbar-->
    </div>
            ...
            <div class="card-body">
                <div class="datatable datatable-bordered datatable-head-custom datatable-default datatable-primary datatable-loaded" id="kt_datatable">
                    <table class="table datatable-column-search-inputs defaultTable">
                        ...
                        <tbody id="sortable">
                            @can('master-policy.perform', ['team-category', 'edit'])
                            @php $edit_access = 1; @endphp
                            @endcan
                            @can('master-policy.perform', ['team-category', 'delete'])
                            @php $delete_access = 1; @endphp
                            @endcan
                            @can('master-policy.perform', ['team-category', 'changeStatus'])
                            @php $status_access = 1; @endphp
                            @endcan
                            @foreach($teamCategories as $index=>$item)
                            <tr id="item-{{ $item->id }}">
                                <td>{{ $index+1 }}</td>
                                <td>{{ $item->title }}</td>
                                <td style="display: none;">{{ !empty($item->parent_id) ? $item->parent->title : 'Parent' }}</td>
                                <td>
                                ...
                                <td class="text-center">
                                    ...
                                    <a title="Edit" href="{{ route('admin.team-category.edit',$item->id) }}" class="btn btn-icon btn-light btn-circle"><i class="la la-pencil"></i></a>
                                    @endif
