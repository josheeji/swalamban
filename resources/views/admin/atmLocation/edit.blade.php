...
@section('title', 'ATM Locations - ' . $atmLocation->title)
@section('styles')
...
<div class="d-flex flex-column-fluid">
    ...
    <div class="container-fluid">
        {!! Form::open(array('route' => ['admin.atm-location.update', $atmLocation->id],'class'=>'form-horizontal','id'=>'validator', 'files' => 'true')) !!}
        <div class="row">
                    ...
                    <div class="card-body">
                        ...
                        <div class="tab-content mt-5">
                            ...
                            <div id="aa-{{ $language['id'] }}" class="tab-pane fade in {{ ($count == 0) ? 'active show' : '' }}">
                                <input type="hidden" name="post[{{ $language['id'] }}]" value="{{ ($language['id'] == $preferredLanguage) ? $atmLocation->id : ($langContent[$language['id']][0]->id ?? "") }}">
                                <div class="form-group {{ $errors->has('title') ? ' has-error' : '' }}">
                                    <label class="control-label">Title <span class="text-danger">*</span></label>
                                    {!! Form::text('title['.$language['id'].']', ($language['id'] == $preferredLanguage) ? $atmLocation->title : ($langContent[$language['id']][0]->title ?? ""), array('class'=>'form-control')) !!}
                                </div>
                                <div class="form-group d-none {{ $errors->has('caption') ? ' has-error' : '' }}">
                                    <label class="control-label">Address</label>
                                    {!! Form::text('address['.$language['id'].']', ($language['id'] == $preferredLanguage) ? $atmLocation->address : ($langContent[$language['id']][0]->address ?? ""), array('class'=>'form-control','placeholder'=>'Address')) !!}
                                </div>
                                <div class="form-group d-none {{ $errors->has('link') ? ' has-error' : '' }}">
                                    <label class="control-label">Ward No.</label>
                                    {!! Form::text('ward_no['.$language['id'].']', ($language['id'] == $preferredLanguage) ? $atmLocation->ward_no : ($langContent[$language['id']][0]->ward_no ?? ""), array('class'=>'form-control')) !!}
                                </div>
                            </div>
                            @php
                            $count++;
                    ...
                    <div class="card-body">
                        <div class="form-group">
                            <div class="checkbox-inline">
                                <label class="checkbox checkbox-lg">
                                    <input type="checkbox" name="is_active" value="1" {{ $atmLocation->is_active == 1 ? 'checked' : '' }}>
                        ...
                        <div class="form-group d-none {{ $errors->has('province_id') ? ' has-error' : '' }}">
                            ...
                            <select name="province_id" class="form-control province">
                                ...
                                <option value="{{ $province->id }}" {{ $province->id == old('province_id', $atmLocation->province_id) ? 'selected' : ''}}>{{ $province->title }}</option>
                                @endforeach
                                @endif
                            </select>
                        </div>
                        <div class="form-group d-none {{ $errors->has('district_id') ? ' has-error' : '' }}">
                            ...
                            <select name="district_id" class="form-control district">
                                ...
                                <option value="{{ $district->id }}" {{ $district->id == old('district_id', $atmLocation->district_id) ? 'selected' : '' }}>{{ $district->title }}</option>
                                @endforeach
                                @endif
                            </select>
                        </div>
                            ...
                            <div class="col-form-label">
                                <div class="radio-inline">
                                    <label class="radio radio-outline radio-success">
                                        <input type="radio" name="inside_valley" value="1" {{ old('inside_valley', $atmLocation->inside_valley) == 1 ? 'checked' : '' }} />
                                        <span></span>
                                        Yes
                                    </label>
                                    <label class="radio radio-outline radio-success">
                                        <input type="radio" name="inside_valley" value="0" {{ old('inside_valley', $atmLocation->inside_valley) == 0 ? 'checked' : '' }} />
                                        <span></span>
                                        No
                                    </label>
                                </div>
                        ...
                        <div class="form-group">
                            ...
                            {!! Form::text('url', $atmLocation->url, array('class' => 'form-control')) !!}
                        </div>
                        <div class="form-group">
                            <label for="" class="control-label">Latitude</label>
                            {!! Form::text('lat', $atmLocation->lat, array('class' => 'form-control')) !!}
                        </div>
                        <div class="form-group">
                            <label for="" class="control-label">Longitude</label>
                            {!! Form::text('long', $atmLocation->long, array('class' => 'form-control')) !!}
                        </div>
                    </div>
                </div>
            </div>
