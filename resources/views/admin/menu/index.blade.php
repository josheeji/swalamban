...
<div class="subheader py-2 py-lg-4  subheader-solid " id="kt_subheader">
    <div class=" container-fluid  d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
        ...
        <div class="d-flex align-items-center">
            @can('master-policy.perform', ['menu', 'add'])
            <a title="Create Content" href="{{ route('admin.menu.create') }}" class="btn btn-default btn-outline-success font-weight-bolder float-right">Create New</a>
            @endif
        </div>
        <!--end::Toolbar-->
    </div>
            ...
            <div class="card-body">
                <div class="datatable datatable-bordered datatable-head-custom datatable-default datatable-primary datatable-loaded" id="kt_datatable">
                    <table class="table datatable-column-search-inputs defaultTable">
                        ...
                        <tbody id="sortable">
                            @can('master-policy.perform', ['menu', 'edit'])
                            @php $edit_access = 1; @endphp
                            @endcan
                            @can('master-policy.perform', ['menu', 'delete'])
                            @php $delete_access = 1; @endphp
                            @endcan
                            @can('master-policy.perform', ['menu', 'changeStatus'])
                            ...
                            <tr id="item-{{ $item->id }}">
                                ...
                                <td>
                                    ...
                                    <a title="Change Status" href="javascript:void(0)" class="btn btn-primary btn-icon btn-circle change-status" id="{{ $item->id }}">
                                        @if($item->is_active == 1)
                                ...
                                <td class="text-center">
                                    ...
                                    <a title="View Menu List" href="{{ route('admin.menu-item.index', $item->id) }}" class="btn btn-info btn-icon btn-circle"><i class="la la-list"></i></a>
                                    <a title="Edit" href="{{ route('admin.menu.edit', $item->id) }}" class="btn btn-icon btn-circle btn-light"><i class="la la-pencil"></i></a>
                                    @endif
                                    @if(isset($delete_access) && $delete_access == true)
                                    <a title="Delete" href="javascript:void(0)" id="{{ $item->id }}" class="btn btn-icon btn-danger btn-circle delete"><i class="la la-trash"></i></a>
                                    @endif
                                </td>
                            </tr>
                            @endforeach
