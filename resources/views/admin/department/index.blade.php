        ...
        $(".defaultTable").on("click", ".change-status", function() {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        type: "POST",
                        url: "{{ route('admin.department.change-status') }}",
                        data: {
                            'id': id,
                        },
                        dataType: 'json',
...
<div class="subheader py-2 py-lg-4  subheader-solid " id="kt_subheader">
    <div class=" container-fluid  d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
        ...
        <div class="d-flex align-items-center">
            @can('master-policy.perform', ['department', 'add'])
            <a title="Create Department" href="{{ route('admin.department.create') }}" class="btn btn-outline-success font-weight-bolder">Create New</a>
            @endif
        </div>
        <!--end::Toolbar-->
    </div>
            ...
            <div class="card-body">
                <div class="datatable datatable-bordered datatable-head-custom datatable-default datatable-primary datatable-loaded" id="kt_datatable">
                    <table class="table datatable-column-search-inputs defaultTable">
                        ...
                        <tbody id="sortable">
                            @can('master-policy.perform', ['department', 'edit'])
                            @php $edit_access = 1; @endphp
                            @endcan
                            @can('master-policy.perform', ['department', 'delete'])
                            @php $delete_access = 1; @endphp
                            @endcan
                            @can('master-policy.perform', ['department', 'changeStatus'])
                            ...
                            <tr id="item-{{ $item->id }}">
                                ...
                                <td class="text-center">
                                    ...
                                    <a title="Edit" href="{{ route('admin.department.edit',[$item->id]) }}" class="btn bg-slate-700 btn-success btn-icon btn-circle legitRipple">
