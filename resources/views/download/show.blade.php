...
@section('content')
    ...
    <section id="pagetitle" style="background-image:url({{isset($menu->image) ?  asset('storage/' . @$menu->image) : asset('swabalamban/images/titlebg.jpg')  }});">
        <div class="container">
            <h1>{{ @$menu->title ?? $category->title ??  'Downloads' }}
            </h1>
            <ul>
                <li>
                ...
                <li>
                    <a href="{{ route('download.index') }}">Download
                    </a>
                    <i class="fas fa-chevron-right">
                    </i>
                </li>
                <li>{{ @$menu->title ?? $category->title ??  'Downloads' }}
                </li>
            </ul>
        </div>
    </section>
                                    ...
                                    @foreach ($downloads as $key => $item)
                                        <tr>
                                            ...
                                            @if (file_exists('storage/' . $item->file) && !empty($item->file))
                                                ...
                                                <td class="text-center">
                                                    <a href="{!! asset('storage/' . $item->file) !!}" class="loding" target="_blank">Download
                                                    </a>
                                                </td>
                                            @endif
                        ...
                        @foreach ($notices as $item)
                            <div class="notice-1">
                                ...
                                <div class="notice">
                                    ...
                                    @if (now()->subDays(2)->format('y-m-d') == $item->start_date->format('y-m-d'))
                                        <span>New
                                        </span>
                                    @endif
